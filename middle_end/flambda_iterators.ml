...
let rec map_return_vars f (t : Flambda.t) : Flambda.t = match t with
  | Var v -> Var (f v)
  | Let (kind, var, def, body) ->
    Let (kind, var, def, map_return_vars f body)
  | Let_rec (bindings, body) ->
    Let_rec (bindings, map_return_vars f body)
  | Switch (var, switch) ->
    Switch(var, {switch with
      consts = List.map (fun (x, y) -> (x, map_return_vars f y)) switch.consts;
      blocks = List.map (fun (x, y) -> (x, map_return_vars f y)) switch.blocks;
      failaction = begin match switch.failaction with
        | Some action -> Some (map_return_vars f action)
        | None -> None end;})
  | String_switch (var, branches, failaction) ->
    String_switch (
      var,
      List.map (fun (x, y) -> (x, map_return_vars f y)) branches,
      begin match failaction with
      | Some action -> Some (map_return_vars f action)
      | None -> None end)
  | Try_with (try_, exception_, catch) ->
    Try_with (
      map_return_vars f try_,
      exception_,
      map_return_vars f catch)
  | If_then_else (if_, then_, else_) ->
    If_then_else (
      if_,
      map_return_vars f then_,
      map_return_vars f else_)
