...
let substitution_aux f sb tree =
  ...
  let aux (flam : Flambda.t) : Flambda.t =
    ...
    | Apply { func; args; kind; dbg; return_arity; } ->
      Apply { func = sb func; args = List.map sb args; kind; dbg; return_arity; }
  ...
  in
  f aux aux_named tree

(* CR mshinwell: change "toplevel" name, potentially misleading *)
(* CR mshinwell: this should use the explicit ignore functions *)
let toplevel_substitution = substitution_aux Flambda_iterators.map_toplevel
let substitution = substitution_aux Flambda_iterators.map
